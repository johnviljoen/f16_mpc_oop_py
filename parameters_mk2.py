...
x_ub = [npos_max, epos_max, h_max, phi_max, theta_max, psi_max, V_max, alpha_max, beta_max, p_max, q_max, r_max, T_max, dh_max, da_max, dr_max, np.infty, lef_max]
x_lb = [npos_min, epos_min, h_min, phi_min, theta_min, psi_min, V_min, alpha_min, beta_min, p_min, q_min, r_min, T_min, dh_min, da_min, dr_min, -np.infty, lef_min]
...
state_vector = stateVector(states,np.copy(x0),x_units,x_ub,x_lb,np.copy(x0),observed_states,mpc_states)                 # dynamics state
input_vector = inputVector(inputs,np.copy(u0),u_units,u_cmd_ub,u_cmd_lb,u_rate_ub,u_rate_lb,np.copy(u0),mpc_inputs)              # dynamic input
simulation_parameters = simulationParameters(dt,time_start,time_end,stab_flag,fi_flag)
